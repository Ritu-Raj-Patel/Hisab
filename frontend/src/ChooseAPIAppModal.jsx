...
function ChooseAPIAppModal({ onSelectUpiApp, onClose, selectedUpiApp }) {
  const [selectedUpiApp, setSelectedUpiApp] = selectedUpiApp || useState(null)
  ...
  const handleContinue = () => {
    if (selectedUpiApp && onSelectUpiApp) {
      onSelectUpiApp(selectedUpiApp)
    }
    if (onClose) {
      onClose()
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex flex-col bg-background-light">
      {/* Header */}
      <div className="flex-shrink-0 px-4 pt-8">
        <header className="flex items-center">
          ...
          <div className="ml-4">
            <h1 className="text-xl font-bold text-content-light">Choose UPI App</h1>
            <p className="text-sm text-subtle-light">Select an app to complete your payment</p>
          </div>
        </header>
      ...
      <main className="flex-1 overflow-y-auto px-4 pt-6 pb-28">
        <div className="space-y-3">
          {upiApps.map((app) => (
            ...
            >
              <img 
                ...
                src={app.logo} 
              />
              <span className="ml-4 font-semibold text-content-light">{app.name}</span>
            </div>
          ))}
          ...
          <div className="cursor-pointer rounded-lg bg-white p-4 shadow-soft flex items-center transition-all duration-200 border-2 border-transparent">
            ...
            <span className="ml-4 font-semibold text-content-light">Add Another UPI ID</span>
          </div>
        </div>
        </main>

      {/* Footer */}
      <footer className="fixed bottom-0 left-0 right-0 z-10 bg-white p-4 border-t border-border-light">
        <div className="flex flex flex-col items-center gap-4">
          <button
            onClick={handleContinue}
            disabled={!selectedUpiApp}
            className={`w-full rounded-full py-3 text-sm font-bold text-black ${
              selectedUpiApp ? 'bg-primary' : 'bg-gray-300 text-gray-500 cursor-not-allowed'
            `}
          >
            Continue
          </button>
          <button
            onClick={onClose}
            className="text-sm font-medium text-subtle-light"
          >
            Cancel
          </button>
        </div>
